...
$cur_index = 1; 
$required_fields = array('req_email' => $lang_common['Email'], 'req_question' => $lang_poll['Question'], 'req_subject' => $lang_common['Subject'], 'req_message' => $lang_common['Message']);
...
<?php
	...
	{
		if (!empty($value))
			echo '<br />'.($ptype == 1 ? '<input name="vote" type="radio" value="'.$key.'" />' : '<input type="checkbox" />').' <span>'.pun_htmlspecialchars($value).'</span><br />';
	}
?>
										<br />
						</fieldset>
...
<?php for ($x = 1; $x <= $pun_config['o_poll_max_fields'] ;$x++) { ?>
						<label><strong><?php echo $lang_poll['Option'] ?></strong><br /> <input type="text" name="poll_option[<?php echo $x; ?>]" value="<?php if (isset($option[$x])) echo pun_htmlspecialchars($option[$x]); ?>" size="60" maxlength="55" tabindex="<?php echo $cur_index++ ?>" /><br /></label>
<?php } ?>
					</div>
				</fieldset>
			</div>
