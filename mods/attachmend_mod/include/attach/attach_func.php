...
function attach_icon($extension){
	...
	return '<img src="'.$icon_url.'" width="15" height="15" alt="Attachment Icon" />';	// return the image stuff...
}
...
function attach_create_subfolder($newfolder=''){
	...
	if(!is_dir($pun_config['attach_basefolder'].$newfolder)){
		...
		if(!mkdir($pun_config['attach_basefolder'].$newfolder,0750))
			error('Unable to create new subfolder with name \''.$pun_config['attach_basefolder'].$newfolder.'\' with mode 0750',__FILE__,__LINE__);
...
function attach_delete_post($id=0){
	...
	$result = $db->query('SELECT af.id FROM '.$db->prefix.'attach_2_files AS af WHERE af.post_id=\''.$id.'\'') or error('Error when searching for all attachments in this post',__FILE__,__LINE__);
