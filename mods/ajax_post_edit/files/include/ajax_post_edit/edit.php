...
<?php if ($is_admmod) : ?>
	...
	<div class="ape-label">
		<label><input type="checkbox" name="silent" id="ape-silent" checked="checked" /> <?php echo $lang_post['Silent edit'] ?></label>
	</div>
...
elseif ($action == "update" && isset($_POST['req_message']) && $id > 0) 
{
	...
	if (isset($_POST['silent']) && $_POST['silent'] == 0 || !isset($_POST['silent']))
	{
	...
	if (empty($errors))
	{
		$sql_edited = ', edited='.time().', edited_by=\''.$db->escape($pun_user['username']).'\'';
		
		if (isset($_POST['silent']) && $_POST['silent'] == 1 && $is_admmod)
			$sql_edited = '';
		
		$result = $db->query('UPDATE '.$db->prefix.'posts SET message=\''.$db->escape($message).'\''.$sql_edited.' WHERE id='.$id) or error('Unable to update post info', __FILE__, __LINE__, $db->error());
