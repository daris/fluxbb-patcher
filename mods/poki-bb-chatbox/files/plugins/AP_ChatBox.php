...
if (isset($_GET['edit_group']))
{
	...
	$result = $db->query('SELECT g_id, g_title, g_read_chatbox, g_post_chatbox, g_post_flood_chatbox FROM '.$db->prefix.'groups WHERE g_id='.$group_id) or error('Unable to fetch user group info', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['add_edit_group']))
{
	...
	$db->query('UPDATE '.$db->prefix.'groups SET g_read_chatbox='.$read_chatbox.', g_post_chatbox='.$post_chatbox.', g_post_flood_chatbox='.$post_flood_chatbox.' WHERE g_id='.$_POST['group_id']) or error('Unable to update group', __FILE__, __LINE__, $db->error());
		...
		<div class="box">
			<form method="post" action="admin_loader.php?plugin=AP_ChatBox.php">
				<div class="inform">
					...
					<fieldset>
						<legend>Ajax Settings</legend>
						<div class="infldset">
						<table class="aligntop" cellspacing="0">
							<tr>
								<th scope="row">Refresh ChatBox</th>
								<td>
									<input type="text" name="form[ajax_refresh]" size="50" maxlength="255" value="<?php echo $pun_config['cb_ajax_refresh'] ?>" />
									<span>How many time between each refresh of the chatbox ? (in second)</span>
								</td>
							<tr>
								<th scope="row">Errors Display (Parsing)</th>
								<td>
									<textarea name="form[ajax_errors]" rows="4" cols="50"><?php echo $pun_config['cb_ajax_errors'] ?></textarea>
									<span>How display the errors messages in your ChatBox. You can use HTML. <strong>Default example:</strong><br />
									<textarea rows="4" cols="50"><span class="conr posted"><pun_date></span><strong><pun_error></strong>: <pun_error_text></textarea>
									</span>
								</td>
							</tr>
						</table>
						...
						<table class="aligntop" cellspacing="0">
							...
							<tr>
								...
								<td>
									...
									<span>Number of maximum messages. Type 0 to keep all messages.</span>
								</td>
							</tr>
							<tr>
								<th scope="row">Messages Display (Parsing)</th>
								<td>
									<textarea name="form[disposition]" rows="4" cols="50"><?php echo $pun_config['cb_disposition'] ?></textarea>
									<span>How display the messages in your ChatBox. You can use HTML. <strong>Default example:</strong><br />
									<textarea rows="4" cols="50"><span class="conr posted"><pun_date></span><pun_admin><pun_guest><strong><pun_username></strong>: <pun_message></textarea>
									</span>
								</td>
							</tr>
						</table>
