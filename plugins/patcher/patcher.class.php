...
class PATCHER
{
	...
	function make_changes()
	{
		...
		if (isset($_SESSION['patcher_config']))
			$fs->put(PUN_ROOT.'patcher_config.php', $_SESSION['patcher_config']);
			...
			foreach ($readme_files as $cur_readme => $cur_steps)
			{
				foreach ($cur_steps as $key => $cur_step)
				{
					if ($cur_step['status'] == STATUS_NOT_DONE)
					{
						...
						$requirements['cannot_open'][$cur_step['code']] = array(0, '', 'Cannot open file <a href="'.PLUGIN_URL.'&show_log#a'.$key.'">#'.$key.'</a>');
					}
					if (isset($cur_step['substeps']))
					{
						foreach ($cur_step['substeps'] as $id => $cur_substep)
						{
							...
							{
								if (!isset($requirements['missing_string']))
									$requirements['missing_string'] = array();

								$requirements['missing_string'][$id.' '.$cur_step['code']] = array(0, '', 'Missing string <a href="'.PLUGIN_URL.'&show_log#a'.$id.'">#'.$id.'</a>');
							}
						}
					}
				}
