...
class Patcher
{
	...
	function __construct($mod)
	{
		...
		$this->config = $this->configOrg = $patcher_config;
					...
					if (isset($curStep['substeps']))
					{
						foreach ($curStep['substeps'] as $id => $curSubStep)
						{
							if (isset($curSubStep['status']) && $curSubStep['status'] == STATUS_NOT_DONE)
							{
								...
								$requirements['missing_strings'][] = array(false, $curStep['code'], $lang_admin_plugin_patcher['Missing string'].' <a href="'.PLUGIN_URL.'&show_log#a'.$id.'">#'.$id.'</a>');
							}
						}
					}
				}
	...
	function stepUpload()
	{
		...
		if ($this->uninstall)
		{
			...
			foreach ($directories as $curDir)
			{
				...
				if ($fs->is_empty_directory(PUN_ROOT.$curDir))
		...
		foreach ($this->mod->filesToUpload as $from => $to)
		{
			if (is_dir($this->mod->readmeFileDir.'/'.$from))
				$fs->copy_directory($this->mod->readmeFileDir.'/'.$from, PUN_ROOT.$to);
